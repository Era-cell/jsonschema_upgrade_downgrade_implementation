import jsonpatch from 'https://cdn.jsdelivr.net/npm/fast-json-patch@3.1.1/+esm';
import jsonschema from 'https://cdn.jsdelivr.net/npm/jsonschema@1.4.1/+esm';
...
function patchschema(obj, patches) {
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // // CTA
    // document.getElementById("goToPatchOnDemand").addEventListener("click", function () {
    //     document.querySelector('html, body').animate({ scrollTop: document.getElementById("section-patch-on-demand").offset().top }, { duration: 800 });
    // });
    // document.getElementById("goToPatchApi").addEventListener("click", function () {
    //     document.querySelector('html, body').animate({ scrollTop: document.getElementById("section-patch-api").offset().top }, { duration: 800 });
    // });

    // Patch on-demand
    document.getElementById('patch-json').addEventListener("click", function () {
        // reset form
        ...
        document.querySelector('.form-group-result').classList.add('d-none');
        document.getElementById('result-textarea').value = '';
        document.querySelectorAll('.alert-response').forEach(function (elem) {
            elem.classList.add('d-none');
            elem.querySelector('.error-message').textContent = '';
        });

        var json_textarea = document.getElementById('json-textarea'),
            json_textarea_val = json_textarea.value,
            patch_textarea = document.getElementById('patch-textarea'),
            patch_textarea_val = patch_textarea.value,
            next = true;

        if (json_textarea_val) {
            if (!IsJsonString(json_textarea_val)) {
                json_textarea.classList.add('is-invalid');
                json_textarea.parentNode.insertAdjacentHTML('beforeend', '<div class="invalid-feedback">Invalid JSON object.</div>');
                next = false;
            }
        } else {
            json_textarea.classList.add('is-invalid');
            json_textarea.parentNode.insertAdjacentHTML('beforeend', '<div class="invalid-feedback">This field is required.</div>');
            next = false;
        }

        if (patch_textarea_val) {
            if (!IsJsonString(patch_textarea_val)) {
                patch_textarea.classList.add('is-invalid');
                patch_textarea.parentNode.insertAdjacentHTML('beforeend', '<div class="invalid-feedback">Invalid JSON object.</div>');
                next = false;
            }
        } else {
            patch_textarea.classList.add('is-invalid');
            patch_textarea.parentNode.insertAdjacentHTML('beforeend', '<div class="invalid-feedback">This field is required.</div>');
            next = false;
        }
        ...
        setTimeout(function () {
            try {
                json_textarea_val = JSON.parse(json_textarea_val);
                patch_textarea_val = JSON.parse(patch_textarea_val);
                var patched = patchschema(json_textarea_val, patch_textarea_val)
                console.log("res", JSON.stringify(patched, null, 4));
                document.querySelector('.form-group-result').classList.remove('d-none');
                document.getElementById('result-textarea').value = JSON.stringify(patched, null, 4);
            } catch (message) {
                document.querySelector('.form-group-result').classList.add('d-none');
                document.querySelector('.alert-response .error-message').textContent = message.errorMessage;
                document.querySelector('.alert-response').classList.remove('d-none');
            }
        }, 500);
